...
function buildJS ( buildJSParams = { } ) {
	const { srcPath, distPath, mainJSFile, es6, minify, moduleName } = buildJSParams;
	...
	return listFiles( srcGlob )
		...
		.then( files => {
			return Promise.all( files.map( file => {
				...
				const targetFilepath = path.normalize( path
				...
				if (
					...
					for ( let search in replacements ) {
						if ( file.fileContent.indexOf( search ) !== -1 ) {
							file.fileContent = file.fileContent.replace( search, replacements[search] );
						}
					}
				}
...
function processCSSFile ( filePath ) {
	return loadFile ( filePath )
		.then( file => {
			return postcss( [ importCss, cleanCss ] )
				.process( file.fileContent, {
					from: filePath
				} );
		} )
		.then( res => {
			const fileName = getFileName( filePath, false ).replace( '.css', '.min.css' );
			const fileContent = res.css
				.replace( /\(..\/..\//gm, '(../' );
