...
export class ImageProcessor {
	constructor () {
		this.canvasWrapperEl = document.querySelector( '.sn-workspace__canvas-wrapper' );
...
function sanitizeParameters ( values ) {
	...
	for ( let key in values ) {
		switch ( key ) {
			case 'blur' :
				result[key] = Math.round( mapRange( parseInt( values[key], 10 ), 0, 100, 1, 100 ) );
				break;

			case 'accuracy' :
				result[key] = mapRange( parseInt( values[key], 10 ), 0, 100, 1, 0.1 );
				break;

			case 'vertex-count' :
				result.vertexCount = Math.round( mapRange( parseInt( values[key], 10 ), 0, 100, 100, 5000 ) );
				break;
		}
		...
		result.strokeWidth = mapRange( parseInt( values['stroke-width'], 10 ), 0, 100, 0, 100 );
	}
