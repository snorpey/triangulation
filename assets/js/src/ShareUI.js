...
export class ShareUI {
	constructor () {
		...
		this.imgurDescriptionInputEl = document.getElementById( 'imgur-description' );
		...
		Array.from( document.querySelectorAll( '[data-callback="startover"]' ) )
		...
		this.uploadBtn.addEventListener( 'click', this.uploadClicked.bind( this ), false );
		this.imgurURLInputEl.addEventListener( 'click', this.selectInput.bind( this ), false );
		...
		this.imgurDescriptionInputEl.addEventListener( 'focus', () => {
			if ( this.imgurDescriptionInputEl.value === this.defaultImgurDescription ) {
		...
		this.imgurDescriptionInputEl.addEventListener( 'blur', () => {
			if ( this.imgurDescriptionInputEl.value === '' ) {
				this.imgurDescriptionInputEl.value = this.defaultImgurDescription;
			}
		} );
	}
		...
		if ( ! this.isUploading ) {
			...
			Promise.resolve()
				.then( () => {
					...
					const delay = remainingDuration > 0
						? remainingDuration
						: 0;
				...
				.then( response => {
					if ( response && response.success ) {
						...
						let errorMessageToDisplay = 'An error occured. Please try again later.';

						if ( response && response.data && response.data.error && typeof response.data.error === 'string' ) {
							errorMessageToDisplay = JSON.parse( JSON.stringify( response.data.error ) );
						}

						this.imgurURLErrorEl.textContent = errorMessageToDisplay;
	...
	imageUploaded ( response ) {
		...
		if (
			...
			const shareText = 'Check out what I made with @snorpey’s triangulation tool';
			...
			const facebookShareURL = `https://www.facebook.com/sharer/sharer.php?s=100&&p[url]=${imageURL}&p[title]=${shareTitle}&p[images][0]=${imageURL}&p[summary]=${encodeURIComponent(shareText)}`;
			...
			this.twitterLinkEl.href = `https://twitter.com/intent/tweet?text=${encodeURIComponent( twitterShareURLText )}`;
			this.facebookLinkEl.href = facebookShareURL;
			this.redditLinkEl.href = `https://www.reddit.com/submit?url=${encodeURIComponent( imageURL )}&title=${shareTitle}`;
		} else {
			this.uploadFailed();
		}
	}
	...
	shareNatively () {
		if ( hasShareAPI ) {
			eventBus.emit( 'export-requested', ( { png } ) => {
				...
				dataURLToFile( imageURL, this.imgurTitleInputEl.value + '.png' )
					.then( file => {
